...
<div class="mt-8">
    <div class="grid grid-cols-2 gap-6">
        <div>
            ...
            <p class="font-light text-gray-500">{{$data->supplier->company}}</p>
        </div>
        <div>
            <Label class="font-semibold">Email</Label>
            <p class="font-light text-gray-500">{{$data->supplier->email}}</p>
        </div>
        <div>
            <Label class="font-semibold">Phone</Label>
            <p class="font-light text-gray-500">{{$data->supplier->phone}}</p>
        </div>
        <div>
            <Label class="font-semibold">Address</Label>
            <p class="font-light text-gray-500">{{$data->supplier->address}}</p>
        </div>
    </div>
        <hr class="mt-8">
...
<button 
    ...
    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 w-full mt-10"
>
Pay
</button>
<!-- Main modal -->
            ...
            <div class="p-4 space-y-4 md:p-5">
                <form action="{{route('accounts_payable.store')}}" method="post">
                    ...
                    <div  class="max-w-sm mt-3">
                        ...
                        <input type="number" min="0" max="{{$data->amount - $totalPayed}}"  name="amount" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light" required />
                    </div> 
